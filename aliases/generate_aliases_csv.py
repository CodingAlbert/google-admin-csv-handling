...
def generate_aliases(animals, adjectives, num_aliases):
    ...
    while len(unique_aliases) < num_aliases:
        ...
        alias = f"{adjective}.{animal}".replace("-", "").replace(" ", "").replace("'", "")  # Remove hyphens, spaces, and apostrophes
...
def main(num_aliases):
    ...
    with open('animal_aliases.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        for alias in aliases:
            writer.writerow([alias])

    print(f"Generated {num_aliases} aliases and saved them to 'animal_aliases.csv'")
